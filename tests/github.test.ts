...
describe('GitHub Events validation and mapping', () => {
	...
	it('retrieves a missing Pull Request title from its validated API URL', async () => {
		const pullRequest = event({
			id: '2',
			type: 'PullRequestEvent',
			payload: {
		...
		const { adapter: github, requests } = adapter([
			...
			response({ number: 4, title: 'Improve docs' }),
		]);
	...
	it('uses the remaining retry budget for Pull Request title enrichment', async () => {
		const pullRequest = event({
			type: 'PullRequestEvent',
			payload: {
				action: 'opened',
	...
	it('retrieves missing merge state for a merged Pull Request event', async () => {
		const pullRequest = event({
			id: '4',
			type: 'PullRequestEvent',
			payload: {
				...
				number: 263,
				pull_request: {
					url: 'https://api.github.com/repos/octocat/hello-world/pulls/263',
				},
			},
		});
		const { adapter: github, requests } = adapter([
			response([pullRequest]),
			response({ number: 263, title: 'Ship it', merged: true }),
		]);
	...
	it('derives closed action from missing merge state in Pull Request details', async () => {
		const pullRequest = event({
			type: 'PullRequestEvent',
			payload: {
				action: 'closed',
				number: 264,
				pull_request: {
					url: 'https://api.github.com/repos/octocat/hello-world/pulls/264',
				},
			},
		});
		const { adapter: github } = adapter([
			response([pullRequest]),
			response({ number: 264, title: 'Close it', merged: false }),
		]);
		...
		expect(result).toMatchObject({
			...
			data: {
				activities: [
					{ family: 'pull-request', number: '264', action: 'closed' },
				],
			},
		});
	});

	it('rejects a merged event whose detail response is not merged', async () => {
		const pullRequest = event({
			type: 'PullRequestEvent',
			payload: {
				action: 'merged',
				number: 265,
				pull_request: {
					url: 'https://api.github.com/repos/octocat/hello-world/pulls/265',
				},
			},
		});
		const { adapter: github } = adapter([
			response([pullRequest]),
			response({ number: 265, title: 'Not merged', merged: false }),
		]);
	...
	it('fails closed without fetching a noncanonical Pull Request URL', async () => {
		const pullRequest = event({
			type: 'PullRequestEvent',
			payload: {
				action: 'opened',
				number: 4,
				pull_request: {
					url: 'https://example.com/repos/octocat/hello-world/pulls/4',
					number: 4,
				},
			},
		});
	...
	it('fails when the Pull Request detail response has no title', async () => {
		const pullRequest = event({
			type: 'PullRequestEvent',
			payload: {
				action: 'opened',
