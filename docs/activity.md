...
| `PullRequestEvent` / `closed` with `payload.pull_request.merged === false` or missing  | Pull requests / `closed`             | `payload.number`, `payload.pull_request.title`, canonical pull-request source URL                                                                                        | Missing merge metadata is interpreted as historical `closed`; merge metadata is not rendered.                                |
| `PullRequestEvent` / `merged`, or `closed` with `payload.pull_request.merged === true` | Pull requests / `merged`             | `payload.number`, `payload.pull_request.title`, canonical pull-request source URL                                                                                        | A `merged` event may omit nested merge metadata; contradictory `merged === false` is malformed.                              |
