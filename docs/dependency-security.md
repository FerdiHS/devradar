...
The repository declares `npm@10.9.2` in `package.json`. CI installs that exact
npm version into a temporary prefix and adds it to `PATH`; contributor
instructions use the same explicit npm installation approach. The `check:npm`
script rejects a dependency-quality run started by a different npm version.
The `packageManager` field remains useful metadata, but it is not evidence that
it controls Dependabot's internal lockfile generator; that generator version is
not recorded in the current update pull requests.

The first Wave 1A CI run exposed an environment-specific limitation: the
Corepack `0.30.0` bundled with Node.js `22.13.0` failed at `corepack npm
--version` with a current npm registry signing key that was absent from its
bundled key set. Node.js `24.19.0` bundled Corepack `0.35.0` and did not show
that failure. Installing npm `10.9.2` directly avoids relying on that bundled
Corepack key set while preserving the supported Node.js matrix.
