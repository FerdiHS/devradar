...
- [x] **Step 5: Commit and push only after fresh verification.** Use focused Conventional Commits, including `chore(release): handle prior approval workflow runs` for release-automation changes, then push `fix/release-approval-prior-runs` with `git push -u origin fix/release-approval-prior-runs`. Do not run `gh pr create` and do not mutate PR #22 or Issue #26.
